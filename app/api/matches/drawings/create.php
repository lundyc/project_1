...
$beforeSecondsRaw = $sanitizedPayload['show_before_seconds'] ?? $sanitizedPayload['before_seconds'] ?? $input['show_before_seconds'] ?? $input['before_seconds'] ?? null;
$afterSecondsRaw = $sanitizedPayload['show_after_seconds'] ?? $sanitizedPayload['after_seconds'] ?? $input['show_after_seconds'] ?? $input['after_seconds'] ?? null;
$beforeSeconds = is_numeric($beforeSecondsRaw) ? max(0, (int)$beforeSecondsRaw) : 5;
$afterSeconds = is_numeric($afterSecondsRaw) ? max(0, (int)$afterSecondsRaw) : 5;
$showFromSecond = max(0, $timestamp - $beforeSeconds);
$showToSecond = $timestamp + $afterSeconds;
$sanitizedPayload['before_seconds'] = $beforeSeconds;
$sanitizedPayload['after_seconds'] = $afterSeconds;
$sanitizedPayload['show_before_seconds'] = $beforeSeconds;
$sanitizedPayload['show_after_seconds'] = $afterSeconds;
$sanitizedPayload['show_from_second'] = $showFromSecond;
$sanitizedPayload['show_to_second'] = $showToSecond;
...
function sanitizeDrawingPayload(array $payload): array
{
          $sanitized = [
                    ...
                    'before_seconds' => isset($payload['before_seconds']) ? (int)$payload['before_seconds'] : null,
                    'after_seconds' => isset($payload['after_seconds']) ? (int)$payload['after_seconds'] : null,
                    'show_before_seconds' => isset($payload['show_before_seconds']) ? (int)$payload['show_before_seconds'] : null,
                    'show_after_seconds' => isset($payload['show_after_seconds']) ? (int)$payload['show_after_seconds'] : null,
          ];
