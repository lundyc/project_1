...
$existingTimestamp = max(0, (int)($annotation['timestamp_second'] ?? 0));
$existingShowFrom = isset($annotation['show_from_second']) ? (int)$annotation['show_from_second'] : max(0, $existingTimestamp - 5);
$existingShowTo = isset($annotation['show_to_second']) ? (int)$annotation['show_to_second'] : $existingTimestamp + 5;
$existingBefore = isset($annotation['show_before_seconds'])
          ? max(0, (int)$annotation['show_before_seconds'])
          : max(0, $existingTimestamp - $existingShowFrom);
$existingAfter = isset($annotation['show_after_seconds'])
          ? max(0, (int)$annotation['show_after_seconds'])
          : max(0, $existingShowTo - $existingTimestamp);
$beforeSeconds = null;
if (array_key_exists('show_before_seconds', $input) && is_numeric($input['show_before_seconds'])) {
          $beforeSeconds = max(0, (int)$input['show_before_seconds']);
} elseif (array_key_exists('before_seconds', $input) && is_numeric($input['before_seconds'])) {
          $beforeSeconds = max(0, (int)$input['before_seconds']);
} else {
          $beforeSeconds = $existingBefore;
}
$afterSeconds = null;
if (array_key_exists('show_after_seconds', $input) && is_numeric($input['show_after_seconds'])) {
          $afterSeconds = max(0, (int)$input['show_after_seconds']);
} elseif (array_key_exists('after_seconds', $input) && is_numeric($input['after_seconds'])) {
          $afterSeconds = max(0, (int)$input['after_seconds']);
} else {
          $afterSeconds = $existingAfter;
}
$showFromSecond = max(0, $timestamp - $beforeSeconds);
$showToSecond = $timestamp + $afterSeconds;

try {
          $updated = annotation_update(
                    $annotationId,
                    $matchId,
                    (int)$timestamp,
                    $showFromSecond,
